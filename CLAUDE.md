...
**Navbar** — `position: fixed`, **full-width and flush to the top** (`top:0; left:0; right:0; border-radius:0`), 3-column grid (logo / centered `.nav-links` / Get KOVA), `isolation:isolate`. The bar itself is `background:transparent; border:none`; the glass lives on a **`.nav::before`** layer (`inset:0; z-index:-1; background:rgba(255,255,255,0.06); backdrop-filter:blur(12px); opacity:var(--nav-glass,0)`) — so at the very top the nav is see-through and **fades into frosted glass over the first 100px of scroll** (`updateNavGlass()` sets `--nav-glass = min(scrollY/100, 1)`, folded into the `_uiRaf` scroll loop). The three **middle nav links were removed** (Round 9) — `.nav-links` is now an **empty container holding only an HTML comment with a copy-paste `.nav-btn` template** (so a link can be re-added in one line and inherit the shared look + scroll-spy). The `.nav-btn` CSS, hover/`.active` animation, and the JS (`refreshNavOffsets`/`updateNavActive`, which simply iterate over the now-zero `.nav-btn[data-target]` set) are all left intact for that future use. Middle links (`.nav-btn`), when present, are **Inter 400**, resting `rgba(255,255,255,0.55)`, easing to `#fff` on hover/`.active` over `0.4s` (no bold/weight jump anymore — removed so section-to-section active changes are smooth). The logo keeps `filter: invert(1) hue-rotate(180deg)` permanently (white text, green wheel). To re-add a link: `<button class="nav-btn" data-target="SECTION_ID" onclick="scrollToSection('SECTION_ID')">Label</button>` inside `.nav-links`. **Get KOVA** (`.nav-btn.primary`) is now an **`<a>`** that links **directly to the App Store** (`https://apps.apple.com/us/app/ridekova/id6757269118`, new tab) — it no longer scrolls to `#download`. It's styled as a **liquid-glass pill with a green-tinted rim + text** (see Buttons) — but because it's a **fixed** element, its backdrop uses a cheap `blur(3px)` (the `url(#container-glass)` SVG filter was **dropped here for perf**: on a fixed pill it re-runs the turbulence filter over the scrolling page every frame). Active state is set by `updateNavActive()` which compares scroll against each target's document-top, **cached** by `refreshNavOffsets()` (called on init, `load`, `resize`, and after each road rebuild — previously it was defined but never called, so the active highlight was stuck on the last link; that's fixed). The Get KOVA link has no `data-target`, so it never gets `.active`. `updateNavActive` + `updateNavGlass` run from the single merged `_uiRaf` scroll loop (which also calls `window._roadScroll`).

**Hero** — `#hero` (z-index 1; not a `.site-section`, no wave clip). `drive pic.jpg` background + dark overlay (`.hero-bg::before`, `rgba(0,0,0,0.35)`). (The faint **`.hero-bg::after` dot grid was removed** in Round 11.) `.hero-content` is the centered text column, nudged **down `transform:translateY(7vh)`** so it sits slightly below dead-center. Title `.hero-title` is white Fraunces `clamp(3rem,7vw,5.5rem)`, reading "Your world just / got <span class="hl">bigger.</span>" — the word **"bigger." is wrapped in `.hl`**, a green-marker highlight: `background-image: linear-gradient(to bottom, transparent 20px, var(--bright-green) 20px)` (square corners; the 20px transparent strip drops the band's top edge below the cap while the bottom stays on the baseline) with `color:#0D1117` black text and `box-decoration-break:clone`. The `.hero-sub` subtext (`margin-bottom:6px`) is followed by a small **`.hero-press-link`** — a muted "Featured on Purdue.edu ↗" text link (Inter, `rgba(255,255,255,0.58)`, underlined, links to the Purdue article in a new tab; `display:block; width:fit-content; margin:0 auto` so it centers, and it `fadeUp`s on load at `0.8s`). **Round 11 removed the old hero CTA + eyebrow entirely:** the `.btn-purdue` "As seen at Purdue" glass eyebrow badge above the title and the `.hero-ctas` row with its "Pull up" button are both gone (the press link replaced the badge as the only Purdue-article link). One decorative **chip image** sits at the lower-right (see below).
...
- `og-image.jpg` — **chat link-preview / share card** (NOT the favicon): a dedicated **1200×630** (≈112 KB) crop of `drive pic.jpg`, biased to its `center 40%` framing. Referenced from the `<head>` Open Graph + Twitter Card meta (`og:image` / `twitter:image`, with `og:image:width/height`, `og:type`, `og:title`, `og:description`, `twitter:card=summary_large_image`, plus a `<meta name="description">`). **Image paths are relative** (`og-image.jpg`) so previews resolve on whatever domain serves the page — the site is on Vercel now, moving to **ridekova.com** later (when that's live, optionally add an absolute `og:url`/`og:image` for the few stricter scrapers). Kept ~1200×630 and well under WhatsApp's ~300 KB preview limit. Regenerate with PIL: crop `drive pic.jpg` (1200×800) to 1200×630 at top = `round((800-630)*0.40)`, `quality=85, optimize, progressive`.
- `drive pic.jpg` — hero background (was also how-it-works; now hero only). 1200×800, ~130 KB. Also the source for `og-image.jpg`.

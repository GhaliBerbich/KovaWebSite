...
**GPU layer promotion** — `will-change: transform, opacity` is set on `.reveal`, `.step-phone-wrap`, `.step-text-col` to pre-promote layers before IntersectionObserver fires, eliminating the jank spike when CSS transitions first start. `will-change: transform` on `.road-brush` and `.testimonials-track`. Step images use `decoding="async"` to move image decode off the main thread.

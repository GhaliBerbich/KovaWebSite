...
9. **JS** — Lenis, nav active + nav glass + bottom blur, letter/word splitters, reveal observer, FAQ accordion, road animation (**how-it-works, testimonials, more-than-a-ride, ambassador**)
...
**Buttons** — `.btn-primary` (green CTA, radius 10) and `.btn-glass` (glass secondary); all buttons use **Inter** (one rule: `button, .btn-primary, .btn-glass, .btn-purdue, .nav-btn { font-family:'Inter', sans-serif }`). Two CTAs are now **liquid-glass** (`backdrop-filter:url(#container-glass)` + inset rim `box-shadow`, springy/`ease` scale on hover): the ambassador **Apply Now** (`.btn-apply`, white rim, scale `1.05` on hover — see Ambassador) and the nav **Get KOVA** (`.nav-btn.primary`, **green-tinted** rim/border + green text `#9af0b8`, subtle scale `1.02` on hover). The hero **"Pull up"** `<button>` calls `scrollToSection('how-it-works')`. **Get KOVA** is an `<a>` linking straight to the App Store. So the App Store URL (`https://apps.apple.com/us/app/ridekova/id6757269118`) is now used by **Get KOVA**, `.appstore-btn`, and the QR image.
...
- **Scroll driver** — `updateTargets()` (rAF-throttled on scroll) paces the draw to the **section's own height** so it works for sections taller than the viewport: `progress = clamp((scrollY - (top - vh/2)) / h, 0, 1)`. So `p=0` when the top edge reaches the viewport middle, `p=1` when the bottom edge does, and `p=0.5` lands when the section center sits at the viewport middle. **For a viewport-height section this is mathematically identical to the old center-crossing formula** (`(scrollY - center + vh)/vh` with `center = top + h/2`), so the short sections (testimonials, ambassador) are unchanged — but the **tall** sections (how-it-works, more-than-a-ride, which exceed `100vh` because of their stacked screenshots) now reveal top→bottom in step with scrolling instead of bursting in the middle / appearing in mid-air. *Earlier dead-ends: a fixed one-viewport center-crossing window left the tall roads drawing mid-section, and a "static fully-drawn" variant looked abrupt — the height-paced formula above is the keeper.*
- **`tick()`** eases `drawnLen → targetLen` (`SMOOTH ≈ 0.14`), sets `strokeDashoffset`, positions/rotates the car at the tip; the car fades out at the very start and once the road is essentially complete.
- **Rebuilds** — `buildAll()` runs on initial load, on `resize`, on `window` `load`, **and on each `.app-shot` image `load`** — because section heights (and therefore `inst.top`/`inst.h` and the document positions of everything below) depend on the lazy screenshots. Without the correct heights the roads draw against stale geometry (the old "appears/vanishes in mid-air" bug); the `width`/`height` attributes on the shots plus these rebuilds keep them correct.
